...
RMC の **Activate / Learn** 段階（還流）で集まる、企業固有の結果データを記録します。

- 直近のパフォーマンスデータ（CVR・CPA・ROAS・LTV など実績値）
- 施策ごとの成功／失敗記録と検証途中の仮説
- まだ profile 層に昇華していない学び（Nが足りない・再現性未確認のもの）

検証が済んで **普遍化できた** 学びは `/learn` スキルが `private/memory/workspaces/<slug>/profile/` に昇華します。
...
- RMC の 5 段階（Listen / Insight / Release / Activate / Learn）と情報層が直接対応する

> 層の名前が `results/` でスキルの名前が `/learn` と分かれているのは、RMC の Activate / Learn 段階（還流）の **保管先** がこの層である、という役割分担を明確化するためです。
