...
SURF の **Fitting** 段階（還流）で集まる、企業固有の結果データを記録します。
...
- SURFの4段階（S/U/R/F）と情報層が直接対応する

> 層の名前が `results/` でスキルの名前が `/feedback` と分かれているのは、SURF の Fitting 段階（還流）の **保管先** がこの層である、という役割分担を明確化するためです。
