...
- **Listen Preflight**: `private/memory/workspaces/active/results/performance-data.md` `private/memory/workspaces/active/profile/*` が存在する前提
...
`private/memory/workspaces/active/profile/customer-signal.md` が未存在なら：

```bash
bin/listen customer
```

Customer Sync への追記先を先に作る。数値だけで Learn を閉じない。
