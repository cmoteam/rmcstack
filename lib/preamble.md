...
スキル実行前に、自分が RMC のどの段階を担っているかを把握してください:

- **Listen 補強**（入力）: ユーザー入力や知識ベースに不足があれば質問で補う（全エージェント共通）。情報・認識・課題のサイロを解消することが目的
- **Insight に応える**（出力）: レビュー / 分析系スキル（`/insight` `/brand` `seo-playbook.md` `cvr-optimization-playbook.md` `web-analytics-practice.md` ほか）は「視点を切り替えて根拠ある選択肢・診断を返す」
- **Release を駆動する**（解放）: `/release` `/insight consultant` は「既成の KPI・聖域・前提を機械的に列挙して手放し議論を起動する」。ただし**実際に手放す決断は人間に残す**
- **Activate / Learn を生む**（適合・本番反映・還流）: 制作系（`content-marketing.md` `digital-advertising.md` `cvr-optimization-playbook.md`）は「そのまま本番で使える成果物」を出し本番反映する。`/learn` は結果を次サイクルの Listen に還流させる
...
2. **実行ベース**: アドバイスだけでなく、実際に使える成果物を出力する。Activate / Learn（本番反映）または還流まで到達させる
...
6. **Listen 不足は明示する**: `knowledge/` または `private/memory/` に `[TODO]` が残っているファイルがあれば、出力の冒頭で「Listen が不足しているため推定で回答している」旨を明記する
...
- 推奨ファイル名: `YYYYMMDD-{skill}-{slug}.{ext}`（例: `20260427-content-marketing-hero-copy.md`）
...
## Knowledge Loading Protocol（= RMC の Listen）

スキル実行時、SKILL.md の `Required Knowledge` セクションに記載されたファイルを読み込んでください。
ファイルに `[TODO]` が残っている場合は、その情報が未設定であることをユーザーに通知し、
可能な範囲で汎用的なアドバイスを行ってください。これが RMC の **Listen** 段階です。
Listen が不十分なまま進めると、以降の Insight / Release / Activate / Learn の精度が全て劣化します。
