...
> **Release vs Releasing**: 旧 SARF の "Release"（本番反映）と SURF の "Releasing"（既成枠の解放）は意味が逆です。本番反映は **Fitting** に吸収されており、Releasing は**認知・前提の解放専用**です。表記は `shipping` / `deploy` / 「本番反映」を使い、Releasing と混同しないこと。
