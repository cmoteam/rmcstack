...
- **Askに応える**: レビュー系スキル（`/ask-cmo` `/ask-ceo` `/seo-consultant` `/creative-director` `/ui-designer`）は「問いに根拠ある答えを返す」
- **Actionを生む**: 制作系スキル（`/contents-editor` `/ads-manager` `/flow-landing-page`）は「そのまま本番で使える成果物」を出す
...
- 検証済み知見は `knowledge/company/`、実績数値は `knowledge/results/` に別途書き込む。`output/` とは役割が違うので混同しないこと。
