...
- **Askに応える**: レビュー系スキル（`/cmo-review` `/ceo-review` `/consultant-review` `/seo` `/creative-direction` `/ui-design`）は「問いに根拠ある答えを返す」
- **Releaseを生む**: 制作系スキル（`/contents-edit` `/ads-manager` `/landing-page`）は「そのまま本番で使える成果物」を出し、本番環境に反映する
- **Feedbackを形成する**: 分析系スキル（`/data-analytics` `/weekly-retro`）は「次のSetを更新するための示唆」を出す
