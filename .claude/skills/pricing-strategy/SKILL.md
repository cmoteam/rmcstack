...
`/ceo-review` が収益性全般を見るのに対し、あなたは **価格構造そのもの** にフォーカスします。

## SURF Alignment

- **Position**: Understanding（価格戦略レビュー・再設計）
- **Ask Subtype**: Design（価格構造のゼロベース設計）＋ Review（既存プランの診断）
- **Sync Preflight**: `business-overview.md`（収益モデル・原価構造・LTV）、`icp.md`（支払い意欲・予算感）、`competitors.md`（競合価格）、`positioning.md`（価値提案）、`performance-data.md`（プラン別 CVR・ARPU・LTV）が前提
- **Fitting Hook**: 価格変更後の CVR / ARPU / Churn / NRR 変化を `/feedback` に戻す。価格は **一度変えると戻しにくい** ため、検証は特に慎重に。小規模テスト → 段階的展開の知見を蓄積
...
価格関連の数字は直接 pull したほうが精度が上がる。詳細は [`knowledge/base/integrations.md`](../../knowledge/base/integrations.md)。

| Service | MCP / API | 用途 | Fallback |
|---------|-----------|------|----------|
| Stripe | `stripe` MCP | プラン別 MRR / ARPU / LTV / プラン移動の観測 | ユーザー CSV |
| HubSpot / Salesforce | 各 MCP | 商談単価・Win/Loss での価格観点・Discount 履歴 | ユーザー CSV |
| Amplitude / Mixpanel | 各 MCP | Value Metric 候補（機能別使用量）の観測 | ユーザー CSV |

**運用ルール**:
- 価格変更の本番反映（Stripe Price / Product の変更）は必ずユーザー承認。Migration Plan を先に出す
- プラン変更履歴は `/feedback` 経由で `private/memory/workspaces/active/results/` にも残す（顧客別の Discount 情報は除く）

## Chaining

- **前工程**: `/ceo-review`（ビジネスモデルと LTV 構造の確認）、`/consultant-review`（価格に関する聖域を揺さぶる）
- **後工程**: `/optimize paywall`（新価格の Paywall 反映）、`/churn-prevention`（値上げ時の Churn 対策）、`/data-analytics`（ARPU/LTV/Churn 計測）→ `/feedback`
