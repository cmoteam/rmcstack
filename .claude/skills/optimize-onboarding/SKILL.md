...
Activation 到達率とライフサイクルメールを直結する。詳細は [`knowledge/base/integrations.md`](../../knowledge/base/integrations.md)。

| Service | MCP / API | 用途 |
|---------|-----------|------|
| Amplitude / Mixpanel / PostHog | 各 MCP | Aha Moment 到達率・D1/D7 リテンション・コホート |
| Customer.io / Mailchimp / SendGrid | 各 MCP | ライフサイクルメール（welcome / nudge / reengagement） |
| Intercom | `intercom` MCP | アプリ内メッセージ・Tour の配信 |
| Stripe | `stripe` MCP | Activation → Paid への転換観測 |

Fallback は従来通り手入力のアクティベーションデータ。

## Chaining

- **前工程**: `/optimize-signup-flow`（登録完了までの最適化）
- **後工程**: `/data-analytics`（リテンション計測・コホート分析）、`/contents-edit`（オンボーディングメール文面）→ `/feedback`
