...
- **Set Preflight**: `icp.md`（支払い動機・価格感度）、`business-overview.md`（収益モデル・プラン構造）、`positioning.md`、`performance-data.md`（Free→Paid 転換率、Upgrade 率、ARPU）が前提
- **Brand Preflight Alert**: `private/memory/organization/brand-guidelines.md` に `[TODO]` が残っている、またはファイル未存在の場合、Paywall UI・コピー生成前に「⚠️ ブランドガイドラインが未整備です。`/set-brand` で整備してから再実行を推奨します」と明示してユーザー確認を取る
- **Feedback Hook**: Paywall 表示後の Conversion 率 / 撤退率 / LTV 変化 / Churn への影響を `/feedback` に戻す。「このタイミングの Paywall が最も効く」というパターンは `private/memory/workspaces/active/profile/` のコア資産
...
課金・行動・アップセル UI を直結する。詳細は [`knowledge/base/integrations.md`](../../knowledge/base/integrations.md)。
