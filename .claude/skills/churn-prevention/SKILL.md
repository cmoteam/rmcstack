...
接続されていれば解約トリガー・顧客会話を直接取れる。詳細は [`knowledge/base/integrations.md`](../../knowledge/base/integrations.md)。
...
**運用ルール**:
- 解約会話の PII（顧客名・メアド）は `private/memory/workspaces/active/results/` に生のまま置かない。匿名化 or 集計
...
- **後工程**: `/data-analytics`（Cohort / Churn 計測）、`/contents-edit`（リエンゲージメントメール文面）→ `/feedback`
