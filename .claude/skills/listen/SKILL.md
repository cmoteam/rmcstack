...
- **Sync Preflight**: なし（Listen は RMC サイクルの起点）
...
| scope | 同期源 | 委譲先 | 書き込み先 |
|-------|-------|--------|-----------|
| `team-org` | Team Sync（組織層） | `bin/set-organization` | `private/memory/organization/` |
| `team-brand` | Team Sync（ブランド層） | `bin/set-brand` | `private/memory/organization/brand-guidelines.md` |
| `team-workspace` | Team Sync（workspace 層） | `bin/set-workspace` | `private/memory/workspaces/active/profile/` |
| `customer` | **Customer Sync** | （対話で `customer-signal.md` を埋める） | `private/memory/workspaces/active/profile/customer-signal.md` |
| `market` | Market Sync（外部揮発情報） | `bin/set-update` | `knowledge/update/` |
...
```bash
bin/set-organization     # team-org
bin/set-brand            # team-brand
bin/set-workspace        # team-workspace
bin/set-update           # market
```
...
Customer Sync は専用 bin スクリプトがない（直接対話で埋める）。以下の Intake で進める：

#### Step 0: Bootstrap

`private/memory/workspaces/active/profile/customer-signal.md` が未存在なら、以下のテンプレ構造で新規作成する：
