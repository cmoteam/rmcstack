...
- **Set Preflight**: `icp.md`（登録動機・不安点）と `performance-data.md`（ステップ別完了率）が前提。ステップ別データが無い場合は、まず `/data-analytics` での計測設計から推奨
- **Brand Preflight Alert**: `private/memory/organization/brand-guidelines.md` に `[TODO]` が残っている、またはファイル未存在の場合、UI・コピー生成前に「⚠️ ブランドガイドラインが未整備です。`/set-brand` で整備してから再実行を推奨します」と明示してユーザー確認を取る
...
ステップ別完了率を直接観測する。詳細は [`knowledge/base/integrations.md`](../../knowledge/base/integrations.md)。
...
- **後工程**: `/optimize-onboarding`（初回ログイン後の Activation 最適化）、`/data-analytics`（ステップ別計測）→ `/feedback`
