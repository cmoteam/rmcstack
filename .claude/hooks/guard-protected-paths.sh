#!/usr/bin/env bash
# PreToolUse guard for CMOFlow protected paths.
#
...
block() {
  local reason="$1"
  echo "[CMOFlow guard] BLOCKED: $reason" >&2
  ...
  case "$rel_path" in
    *.md|*.html|*.css|*.pdf|*.csv|*.json)
      ...
      case "$rel_path" in
        ...
        *)
          echo "[CMOFlow guard] WARN: 一次成果物はルート直下ではなく private/output/ 配下に置いてください（CLAUDE.md §Output Directory）。path: $rel_path" >&2
          ;;
      esac
      ;;
  esac
