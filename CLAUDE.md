...
|------|-----------------|
| Set | `/set-organization` `/set-workspace` `/set-update` `/sarf-check` + `knowledge/`（base / update）と `private/memory/`（organization / profile / results） |
...
```
Set（情報を渡す）      → /set-organization /set-workspace /set-update + knowledge/（base） + private/memory/（organization / per-project）
...
- `/workspace` — アクティブ workspace の一覧・切替・新規作成（`private/memory/workspaces/<slug>/` と `private/memory/workspaces/active` symlink を操作）。事業部・クライアント・プロダクト等、SARF Set が独立して成立する単位ごとに分離する。

### SARF Ops（サイクル運用）
- `/set-organization` — Set段階（organization層）。組織全体の情報（ミッション・ビジョン・事業ポートフォリオ）を対話でヒアリングし `private/memory/organization/organization-overview.md` を埋める
- `/set-brand` — Set段階（brand層）。組織共通のブランドガイドライン（トーン・ビジュアル・表記ルール）を対話でヒアリングし `private/memory/organization/brand-guidelines.md` を埋める。クリエイティブ生成系スキルが brand-guidelines 未整備時にアラート経由で誘導する
- `/set-workspace` — Set段階（workspace層）。アクティブ workspace の情報（事業概要・ICP・Positioning・競合）を対話でヒアリングし `private/memory/workspaces/active/profile/` を埋める
...
- **入力**: ユーザーの組織・ミッション・事業ポートフォリオに関する情報（崩れた文体でも可）
...
- **入力**: トーン・ビジュアル・表記ルール・Logo Usage・OK/NG表現（崩れた文体でも可）
...
情報は **`knowledge/`（共有ベース / tracked）** と **`private/memory/`（プロジェクト固有 / gitignored）** の2つのルートに分離されています。ひと目で「upstream に流れるか／流れないか」が判別できる構造です。
...
private/memory/      — per-workspace memory (gitignored)
  ...
  workspaces/active  → <slug>  (symlink, アクティブ workspace を指す / 全スキルが参照)
...
全スキルは `private/memory/workspaces/active/profile/...` `private/memory/workspaces/active/results/...` というパスで memory を読み、`private/memory/workspaces/active` が **アクティブ workspace のディレクトリへの symlink** として機能します。workspace 切替は `/workspace switch <slug>` で symlink を張り替えるだけ（SKILL.md 側の変更は不要）。共通情報の重複は許容する設計（inheritance を導入すると SKILL.md 全体にマージの責務が波及するため意図的に避けています）。
...
`/set-update` で更新する。自社の実績数値（CVR・CPA 等）はここではなく `private/memory/workspaces/active/results/` 側。

### Organization（組織全体で共有する知識 / gitignored）
`private/memory/organization/` — 組織全体のミッション・ビジョン・バリュー、事業ポートフォリオ、組織共通のブランドガイドライン。
全 workspace に共通して適用される上位コンテキスト。`/set-organization` で初期セットアップ・更新。単一インスタンス（workspace のような複数持ちはしない）。

> `private/memory/organization/` は **gitignore 対象**。upstream には共通テンプレート `memory/organization.example/` のみが入っています。初回セットアップは `cp -r memory/organization.example private/memory/organization` または `/set-organization` 経由。`memory/organization.example/` は書き換え禁止（upstream に流れます）。

### Profile（workspace 固有の事業プロフィール / gitignored）
`private/memory/workspaces/active/profile/` — workspace 単位の事業概要、ICP、ポジショニング、競合情報。ブランドガイドラインは組織層（`private/memory/organization/brand-guidelines.md`）に集約されており、workspace 固有の差分が必要になったら `private/memory/workspaces/active/profile/brand-guidelines.md` を追加して override する。
**実体は `private/memory/workspaces/<active>/profile/`** で、`private/memory/workspaces/active` はアクティブ workspace ディレクトリへの symlink。workspace 初期セットアップ時にユーザーが記入し、施策レビュー時に必ず参照される。

> `private/memory/workspaces/` は **gitignore 対象**（各 workspace のローカル管理）。upstream には共通テンプレート `memory/profile.example/` のみが入っています。新規 workspace は `/workspace new <slug>` で作成（`.example/` からテンプレート注入＋ symlink 張り替え）、既存 workspace への切替は `/workspace switch <slug>`。実情報は必ず `private/memory/workspaces/<slug>/profile/` 側に書き、`memory/profile.example/` は書き換えないでください（upstream に流れます）。

### Results（workspace 固有の結果ログ / gitignored）
`private/memory/workspaces/active/results/` — 直近のパフォーマンスデータ（CVR・CPA・ROAS・売上 等）、施策ごとの検証ログ、普遍化前の観測仮説。
**実体は `private/memory/workspaces/<active>/results/`** で、`private/memory/workspaces/active` はアクティブ workspace ディレクトリへの symlink。**中から出てくる** 機密性の高い数値のため gitignore 対象。SARF の Feedback 段階で `/feedback` スキルが書き込む保管先。
...
- **`scope: company-wide`** — `private/memory/workspaces/*/` を全走査。全 workspace を横断してレビューする経営層の役割（CEO / Group CMO）。
- **`scope: workspace`** — アクティブ workspace（`private/memory/workspaces/active/profile/` symlink の指す単一 workspace）だけをレビュー。事業部マネージャー / 専門職の役割。
...
全スキルの **一次成果物**（原稿・HTML/CSS・広告コピー・分析レポート・見積書等）は必ず `private/output/<workspace>/` 配下に書き込みます。`<workspace>` はアクティブ workspace の slug（`readlink private/memory/workspaces/active` で取得）。

- `private/output/` は **gitignore 対象**（`.gitkeep` と `README.md` のみ tracked）。企業固有のコピー・数値を安全に置けます。
- **必ず `private/output/<workspace>/...` に書く**。workspace 単位に分離することで、複数クライアント・複数事業部を並行で扱う際の成果物の誤配・混在を防ぐ。
- プロジェクトルートや `.claude/skills/` 内に直接ファイルを作らないこと。
- 推奨ファイル名: `YYYYMMDD-{skill}-{slug}.{ext}`（例: `private/output/acme-saas/20260419-contents-edit-hero-copy.md`）。
- 成果物を `knowledge/` に書くのは NG。knowledge は「次サイクルの Set に効く蓄積」専用、`private/output/` は「その瞬間の成果物」。役割が違うので混同しないこと。

詳細は [`private/output/README.md`](private/output/README.md) を参照。
...
5. **知識を分離する** — 普遍的な知識と揮発性の情報を混ぜない。SetとFeedbackが汚染されないよう、検証済みの知見のみを `private/memory/workspaces/active/profile/` に記入する。
...
2. `/set-brand` — 組織共通のブランドガイドラインを一括ヒアリング（Set, brand層）
3. `/set-workspace` — workspace 固有情報を一括ヒアリング（Set, workspace層）
4. `/next` — 次の一歩を確認（詳細が必要なら `/sarf-check`）。不足があれば `/set-organization` / `/set-brand` / `/set-workspace` に戻る
5. `/cmo-review`（Ask 単体）or `/campaign-launch`（Set〜Feedback を横断する統合ワークフロー）— 施策レビュー／全工程開始
6. `/data-analytics`（Feedback 単体）or `/weekly-retro`（Feedback 起点で次サイクルの Set へ閉じる統合ワークフロー）— 結果集約
7. `/set-update` + `/feedback` — Set層への還元（サイクルを閉じる）
