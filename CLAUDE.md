...
| 層 | 名前 | 役割 |
|----|------|------|
| サイクル | **RMC** | マーケ組織がどう学習し続けるかの OS |
| 管理対象 | **MVDF** | サイクル内で何を管理・指標化するか（`mvdf.md`） |

RMC が「**どう回すか**」、MVDF が「**何を管理するか**」を担います。

## Available Skills

**RMC 各段に 1 skill ずつ + Ops 2 個 = 7 skills**。詳細は `knowledge/base/agent-catalog.md`。
...
knowledge/           — shared base (tracked, upstream に流れる)
  base/                 不変フレームワーク・マインドセット（SURF・組織学習・責任設計）
...
初回セットアップ: `/workspace init` で `private/` ブートストラップ、`/workspace new <slug>` で workspace 追加（内部で `bin/init-private` `bin/workspace` を呼ぶ）。
...
初回プロジェクトの推奨順（skill 名は再設計後に更新予定）:

0. workspace を 1 つ作成（実在する事業名で）
1. 組織情報を入力（Listen / Team Sync）
2. ブランドガイドラインを入力（Listen / Team Sync）
3. workspace 固有情報を入力（Listen / Team + Customer Sync）
4. 次の一歩確認。不足があれば 1〜3 に戻る
5. Insight（CMO / CEO / SEO / Creative 視点でのレビュー）or 横断ワークフロー（未実装）
6. Release（既成 KPI / 聖域 / 前提の手放し）
7. Activate（広告運用・コンテンツ・CVR 改善・分析等は `knowledge/base/` プレイブックを参照しながらインライン対話で shipping）
8. Learn（外部揮発情報の更新 + 結果の還流。次サイクルの Listen 入力を作る）
...
- `knowledge/base/mvdf.md` — MVDF（Marketing Value Delivery Framework）。RMC が「どう回すか」、MVDF が「何を管理するか」を定義する対の関係
