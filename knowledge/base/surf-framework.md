...
AI に委ねる時代（2026 年〜）には、個人レベルの最適化では足りない。**チーム単位で学習プロセスを埋め込まないと、AI は責任放棄の道具になる**（詳細は `learning-organization.md` `responsibility-design.md` `marketing-structural-issues.md`）。

SURF は OS の主体を**組織 ↔ AI**に置く:
...
| 起点 | チームで情報を**共有する** |
| 中核問い | チームが何を理解しているか |
| 中盤動詞 | Releasing（既成枠の解放） |
| 終端 | Fitting（適合させる ＋ 次サイクル起点） |
| 思想 | 学習する組織 |
...
**Syncing の最上流 — 前提の共有**: データや KPI を揃える前に、もっと根本的な前提を揃える必要がある。**チームが「マーケティング」という言葉で同じことを指しているか**、**個々のメンバーが自分の役割を事業の現在地に紐付けて語れるか**、**戦略から切り離された戦術チームになっていないか**。これが揃っていない状態で数値や施策の議論を始めても、議論はそもそも噛み合わない（詳細は `marketing-structural-issues.md` Section 0）。
