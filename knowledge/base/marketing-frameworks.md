...
世界で広く使われる **51 件のマーケティング・フレームワーク** を網羅カタログ化しつつ、**RMC**（Listen / Insight / Release / Activate / Learn、読み:「アール・エム・シー」）サイクルで実際に呼び出す代表手法は深掘りで解説する。

- **網羅カタログ**: 「どの診断/設計/運用/計測の道具を選ぶか」を決めるためのリファレンス（後述「世界のフレームワーク総覧」セクション）
- **RMC 実装の深掘り**: AARRR / JTBD / STP / RAM-CE / Hook Model / PMF / Lean / ユニットエコノミクス / North Star Metric など、本ナレッジベース内で繰り返し参照される手法を詳述

RMC の各段階での使い分け:
- **Listen**: STP / JTBD / VPC / 3C / Empathy Map / Brand Identity Prism（誰に・何の仕事のために売るかを共有）
- **Insight**: SWOT / Five Forces / Blue Ocean / Perceptual Map / Kano / Hierarchy of Effects / FCB Grid / STDC / CDJ / SERVQUAL / Gap Model / AARRR / RAM-CE / Content Marketing Matrix（どのレンズで診断するか）
- **Release**: 既成の North Star・AARRR・KPI を**意図的に疑い直す**（`/release` の触媒）
- **Activate / Learn**: 4P / 7P / IMC / RACE / SOSTAC / Stage-Gate / BMC / NPS / CLV / Balanced Scorecard / Hook Model / MVP / North Star Metric（何を実装し、何を測って次サイクルに還流するか）

RMC 本体は `knowledge/base/reframing-marketing-cycle.md`、企業固有名の体系は `knowledge/base/signature-frameworks.md`、責任設計は `knowledge/base/responsibility-design.md` を参照。
...
- `knowledge/base/reframing-marketing-cycle.md` — RMC の各段階の責務とスキル対応（マーケ活動 8 領域の Listen 充足チェックリスト含む）
