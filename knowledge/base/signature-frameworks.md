...
**要点**: Google の thought leadership framework としての性格が強い。**RMCStack で使うとき**: `seo-playbook.md` と `content-marketing.md` の上流で、「購買前のどの瞬間に情報提供者として存在するか」の設計レンズになる。
...
**要点**: 単なる思想ではなく、CRM・自動化製品群と結びついて **go-to-market system** として商業化された点が示唆的。近年は Loop Marketing を打ち出すが、Inbound は基礎理論として残りつつ AI 時代に拡張中。**RMCStack で使うとき**: BtoB SaaS workspace の `content-marketing.md` `seo-playbook.md` の根本骨格として参照。
...
**要点**: 短期 KPI ではなく、長期のブランド蓄積を支える **operating philosophy**。**RMCStack で使うとき**: 店舗・コミュニティ型 workspace の `/brand` レンズに加え、「短期効率 vs 滞在価値」の `/release` 触媒として使える。
...
**要点**: 厳密には生産・オペレーション手法だが、Toyota の市場価値・品質信頼・納期安定・コスト競争力の源泉として **マーケティング成果に直結する「見えにくいマーケティング基盤」**。**RMCStack で使うとき**: 「マーケ単独で勝とうとして供給・品質・コスト競争力の劣後を見落としていないか」を `/insight consultant` `/release` で問う材料。
...
2. 現代の固有名手法は **広告キャンペーンの手順書ではなく、企業が需要を作り続けるための operating system** に近い。RMCStack の文脈ではこの含意がそのまま、**RMC が単なる広告手順書ではなく組織の Operating Cycle として 8 領域（市場理解 / 戦略 / ブランド / 提供価値 / チャネル / 需要創出 / 顧客関係 / 測定）を横断する**設計思想に重なる。
...
| スキル | 主に参照すべき手法 | 使う場面 |
|--------|------------------|---------|
| `/insight ceo` | Brand Management、Desire at Scale、One Brand、Universalization | ブランド単位の責任設計・global × local 戦略レビュー |
| `/insight ceo` | Working Backwards、TPS | ROI 視点・供給/品質起点の市場優位 |
| `/insight consultant` | TPS、Universalization、CDA | 既成枠の手放しと再設計 |
| `/brand` | One Brand、Desire at Scale、Third Place | masterbrand 統合・体験設計 |
| `seo-playbook.md` | ZMOT、Inbound Marketing | 購買前の意思決定瞬間と検索接点 |
...
| `/release` | TPS、Third Place、Working Backwards | 短期効率の聖域を疑う触媒 |
| `/listen team-brand` | Brand Management、One Brand、Universalization | ブランドガイドライン・統合度の意思決定 |
| `web-analytics-practice.md` | ZMOT、CDA | 検索データの市場調査化、デジタル KPI 統合 |

## 関連ドキュメント

- `knowledge/base/reframing-marketing-cycle.md` — RMC サイクル本体（マーケ活動 8 領域の Listen 充足チェックリスト含む）
