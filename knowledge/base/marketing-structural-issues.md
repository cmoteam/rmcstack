...
| 病理 | SURF の応答 |
|------|------------|
| **前提共有の不在**（個人の存在意義・「マーケティング」の共通定義の欠如） | **Syncing**: チーム全員で「自分たちはどこにいて、何のためにいるか」を同じ言葉で言語化する。`/set-organization` `/set-workspace` は職能ではなく事業課題の言語化を強制する |
...
| **戦略と戦術の双方向の断絶**（現場が「なぜ」を見失い、上層部が「どう」を見失う／指標固執と施策の自己目的化） | **Releasing → Fitting**: 引き継ぎ KPI・聖域を手放し（Releasing）、自社の現在地に適合した課題・戦術・目的に再設定する（Fitting）。戦略の失敗を戦術で覆い隠す逃避も、戦術の失敗を戦略不在のせいにする逃避も、両方向の指差しを禁じる |
