...
マーケティングが機能不全に陥る最大の原因は、責任の所在の不明瞭さである（詳細は `marketing-structural-issues.md`）。RMC は技術的な手順書である以前に、**責任を曖昧にしないための設計図**として機能する。本書では、責任の所在を設計する際の原則と判断軸を記録する。
...
マーケティングでは「みんなで話し合って決めました」が常態化しやすく、両者が同時に曖昧になる。RMC では各 workspace ごとに「誰がアカウンタブルか」を `business-overview.md` に記載することを推奨する。
...
| 担い手 | 何を任せるか | 残すべき責任 |
|--------|------------|-------------|
| **インハウス（人間）** | 戦略判断、ブランド整合性の最終判定、Release（既成枠を手放す決断）、Activate / Learn の最終承認 | アカウンタビリティのすべて |
...
RMC の各段階で、誰が何の責任を持つかの基本配分:

| RMC 段階 | 責任主体 | AI の役割 |
|-----------|---------|----------|
| **Listen** | チーム全員（情報を場に出す責任） | サイロ検出、未共有情報の指摘、共有テンプレート提供 |
| **Insight** | 各レビュー観点の担当者（CMO/CEO/Specialist） | 視点の切り替え補助、選択肢生成、抜け漏れ指摘 |
| **Release** | 意思決定者（既成枠を手放す決断は人間に残す） | 「聖域」「前提」「ねばならない」の機械的列挙 |
| **Activate / Learn** | 実行責任者（本番反映と測定の責任） | 実装の叩き台生成、測定設計の補助 |

**Release は人間にしかできない**。既成の KPI を手放す、聖域を否定する、組織政治を超えて「これは違う」と言う ── これらは AI には引き受けられない判断である。
...
このチェックが通らないうちは、責任の所在は構造的に曖昧であり、RMC サイクルは機能しない。
