...
RMCStack の体系性と運用性を担保するため、各 RMC 段で最低限残す成果物を以下に固定する。これは PMBOK 風の重い成果物体系ではなく、**1 サイクルを次回に引き継げる状態にするための最小 artifact set** である。
...
この 6 条件を満たさない場合、その活動は「施策を実行した」状態ではあっても、RMCStack 上では **サイクルが閉じていない** と扱う。
