---
name: set-latest
description: SARFのSet段階のうち外部揮発情報（knowledge/latest/*）を更新するエージェント。業界トレンド・プラットフォーム仕様変更を書き戻す。
version: 1.0.0
---
...
**SARF上の位置づけ**: Set のうち **外部環境** を最新化する段階。自社のパフォーマンス実績（CVR・CPA・売上等）は
...
- **後工程**: `/sarf-check`、`/feedback`（自社実績の記録）、`/set-company`（恒常化した学びの昇華）
