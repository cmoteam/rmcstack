...
あなたは SARFStack のインテイク担当エージェントです。
...
`memory/company/` は **gitignore 対象**（各プロジェクトのローカル管理）です。
リポジトリをクローン直後は存在しないので、最初にテンプレートから複製します。

1. `memory/company/` が存在するか確認する（`ls memory/company/` が失敗したらbootstrap必要）
2. 存在しなければ Bash で以下を実行:
   ```bash
   cp -r memory/company.example memory/company
   ```
3. 復旧後に `memory/company/*.md` を改めて Read する

以降の Step 1〜4 は `memory/company/`（テンプレート側ではなく）に対して行います。
`memory/company.example/` は絶対に書き換えないでください（upstream に流れます）。
...
- foundation/ や update/ を書き換える（このスキルの責務外）
- `memory/company.example/` を書き換える（upstreamに流れる共通テンプレートなので、企業情報は必ず `memory/company/` 側に書く）
