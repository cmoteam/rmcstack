...
Phase 2: Creative     → /ask-creative-director + /content + /ads（制作）
Phase 3: Technical    → /seo + /ui-review（技術最適化）
Phase 4: Executive    → /ask-ceo（経営承認）
Phase 5: Launch       → 最終チェックリスト + ローンチ
Phase 6: Measure      → /analytics（効果測定）→ knowledge/results/ に還元
```
...
- [ ] `knowledge/results/performance-data.md` に直近のベースライン数値があるか
...
**エージェント**: `/ask-creative-director` + `/content` + `/ads`

Phase 1の戦略書に基づき、必要なクリエイティブを制作する。

### 2a: コンセプト開発（`/ask-creative-director`）
...
### Quality Gate
`/ask-creative-director` がすべての制作物をレビュー:
- ブランド一貫性チェック
- メッセージ品質チェック
...
/analytics（分析）    →  /ask-cmo（判断）→  /ads or /content（改善実行）→  /analytics（再分析）
```

キャンペーン終了時には、得られた知見を `knowledge/results/performance-data.md` に書き戻してサイクルを閉じてください。
