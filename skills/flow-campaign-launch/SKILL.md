...
Phase 3: Technical    → /seo-consultant + /ui-designer（技術最適化）
Phase 4: Executive    → /ask-ceo（経営承認）
Phase 5: Launch       → 最終チェックリスト + ローンチ
Phase 6: Measure      → /data-analyst（効果測定）→ knowledge/results/ に還元
```
...
**エージェント**: `/seo-consultant` + `/ui-designer`

### 3a: SEO最適化（`/seo-consultant`）
- LP/コンテンツのSEO最適化
- メタタグ、構造化データの設定
- 内部リンク設計
...
/data-analyst（分析）  →  /ask-cmo（判断）→  /ads-manager or /contents-editor（改善実行）→  /data-analyst（再分析）
```
