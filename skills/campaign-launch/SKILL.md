...
- **[Optional] Campaign Segment**: キャンペーンの Primary Segment。指定があれば `/cmo-review` `/contents-edit` `/ads-manager` 等に `Target Segment` として引き継ぐ
...
Phase 2: Creative     → /creative-direction + /contents-edit + /ads-manager（制作）
Phase 3: Technical    → /seo + /ui-design（技術最適化）
Phase 4: Executive    → /ceo-review（経営承認）
Phase 5: Launch       → 最終チェックリスト + ローンチ（Release）
Phase 6: Measure      → /data-analytics（効果測定）→ memory/results/ に還元
```
...
**エージェント**: `/creative-direction` + `/contents-edit` + `/ads-manager`

Phase 1の戦略書に基づき、必要なクリエイティブを制作する。

### 2a: コンセプト開発（`/creative-direction`）
- キャンペーンのクリエイティブコンセプト策定
- メッセージのフレームワーク（What to say / How to say）
- トーン&マナーの方向性確定

### 2b: コンテンツ制作（`/contents-edit`）
...
### Quality Gate
`/creative-direction` がすべての制作物をレビュー:
- ブランド一貫性チェック
- メッセージ品質チェック
...
/data-analytics（分析）  →  /cmo-review（判断）→  /ads-manager or /contents-edit（改善本番反映）→  /data-analytics（再分析）
```
