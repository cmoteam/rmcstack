...
> **このディレクトリ（`memory/results.example/`）はテンプレートです。**
> 実際のパフォーマンス数値・学びは `memory/results/`（gitignore対象）に格納します。
...
検証が済んで **普遍化できた** 学びは `/feedback` スキルが `memory/company/` に昇華します。
ここ（`results/`）は「生データ／途中段階の観測」の保管庫と考えてください。

## なぜ update/ から分離しているか

- `update/` — 外から入ってくる共有情報（プラットフォーム仕様・業界トレンド）。公開可能で upstream で共有されうる情報。
...
- SARFの4段階（S/A/R/F）と knowledge + memory の4層（foundation / company / update / results）が直接対応する
...
- **必ず `memory/results/` に書く**。`memory/results.example/` に書くと upstream に流れます。
- 検証済みで普遍化できる学びは **`memory/company/` に昇華**。ここ（results/）には生データを残す。
