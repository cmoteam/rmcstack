...
| Feedback | 分析系スキル（`/data-analytics`）＋ `/feedback`（検証ゲート付きで results/ 生データと company/ 検証済み知見に還元） |
...
- **知識**: foundation + company（ICP/Positioning/competitors）+ results
...
- **知識**: foundation + company（company-overview/competitors）+ update + results
...
3. `/cmo-review`（Ask 単体）or `/campaign-launch`（Set〜Feedback を横断する統合ワークフロー）— 施策レビュー／全工程開始
4. `/data-analytics`（Feedback 単体）or `/weekly-retro`（Feedback 起点で次サイクルの Set へ閉じる統合ワークフロー）— 結果集約
